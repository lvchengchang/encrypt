...
var deUploadTemplate = template.Must(template.ParseFiles("decrypt.html"))
...
func decrypt(w http.ResponseWriter, r *http.Request) {
	if err := deUploadTemplate.Execute(w, nil); err != nil {
		log.Fatal("Execute: ", err.Error())
		return
	}
}
...
func deUploadHandle(w http.ResponseWriter, r *http.Request) {
	r.ParseMultipartForm(32 << 20)
	file, handler, err := r.FormFile("file")
	if err != nil {
		fmt.Println(err)
	...
	defer file.Close()
	f, err := os.OpenFile("./de_uploads/"+handler.Filename, os.O_WRONLY|os.O_CREATE, 0666) // 此处假设当前目录下已存在test目录
	if err != nil {
		return
	}
	defer f.Close()
	...
	file, err = os.Open("./de_uploads/" + handler.Filename + ".src")
	if err != nil {
		fmt.Println(err)
		return
	}
	defer file.Close()
	content, err := ioutil.ReadAll(file)
	fileNames := url.QueryEscape("./de_uploads/" + handler.Filename + ".src") // 防止中文乱码
	w.Header().Add("Content-Type", "application/octet-stream")
	w.Header().Add("Content-Disposition", "attachment; filename=\""+fileNames+"\"")
...
func main() {
	...
	http.HandleFunc("/de_uploads", deUploadHandle)
